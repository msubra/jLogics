...
public class Test4 {
	...
	public static void main(String[] args) {
		System.out.println(MathParser.processEquation("a+b"));
	}
}
...
class MathParser {
	...
	private static int evaluate(String leftSide, char oper, String rightSide)
		...
		if (operatorLoc > 0 && operatorLoc < leftSide.length() - 1) {
			leftResult = evaluate(leftSide.substring(0, operatorLoc), leftSide
					.charAt(operatorLoc), leftSide.substring(operatorLoc + 1,
					leftSide.length()));
		} else {
			try {
		...
		if (operatorLoc > 0 && operatorLoc < rightSide.length() - 1) {
			rightResult = evaluate(rightSide.substring(0, operatorLoc),
					rightSide.charAt(operatorLoc), rightSide.substring(
							operatorLoc + 1, rightSide.length()));
		} else {
			try {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("The result of your equation (" + equation
					+ ") is: " + result);
